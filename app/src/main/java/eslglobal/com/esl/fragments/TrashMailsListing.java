...
public class TrashMailsListing extends ParentFragment implements IItemHandler {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		((TextView) layout.findViewById(R.id.catgr_txt)).setText(R.string.notrash);
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		...
		if (!bsecure.isDrawerOpen()) {
			inflater.inflate(R.menu.trashlisting_menu, menu);
	...
	private void getData(int requestId, int currentNo) {
		...
		try {
			...
			String link = bsecure.getPropertyValue("gettrashmaillist");
	...
	private void searchContent(String searchKey) {
		...
		try {
			...
			// HTTPBackgroundTask task = new HTTPBackgroundTask(this,
			// getActivity(), 1, 1);
			String link = bsecure.getPropertyValue("searchInTrash");
			JSONObject object = new JSONObject();
		...
		try {
			...
			case 1:
				...
				if (results != null) {
					...
					Vector<Item> items = (Vector<Item>) item.get("trash_detail");

					if (items == null)
						items = (Vector<Item>) item.get("trash_detail");
	...
	public String getFragmentName() {
		return "Trash";// getString(R.string.trash);
	}
	...
	public void onPrepareOptionsMenu(Menu menu) {
		...
		if (tempId == 1) {
			...
			MenuItem cm_undo_done = menu.findItem(R.id.cm_undo);
			cm_undo_done.setVisible(true);
			...
			MenuItem cm_undo_done = menu.findItem(R.id.cm_undo);
			if (cm_undo_done != null)
				cm_undo_done.setVisible(false);
		}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.cm_delete_done:
			...
			moveMailFromTrash(true); // true = delete, false = undo
	...
	private void moveMailFromTrash(boolean type) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (item.getAttribute("checked").equalsIgnoreCase("y")) {
				comid = comid + "," + item.getAttribute("trashid");
			}
		}
		...
		if (type) { /* true = delete, false = undo */
			url = AppSettings.getInstance(bsecure).getPropertyValue("removeMailFromTrash");
			//url = url.replace("(COMID)", "");
		} else {
			url = AppSettings.getInstance(bsecure).getPropertyValue("moveMailFromTrash");
			//url = url.replace("(COMID)", "");
		}
		...
		if (object != null) {
			...
			if (jsonObject.optString("status").equalsIgnoreCase("0")) {
				...
				for (int i = 0; i < temp.length; i++) {
					adapter.removeTrashItem(temp[i]);
				}
				adapter.notifyDataSetChanged();
