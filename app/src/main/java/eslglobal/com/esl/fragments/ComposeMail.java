...
import eslglobal.com.esl.controls.ProgressControl;
import eslglobal.com.esl.tasks.FileUploader;
import eslglobal.com.esl.tasks.HTTPostJson;
import eslglobal.com.esl.utils.TimeUtils;
...
public class ComposeMail extends ParentFragment implements IItemHandler, IDownloadCallback {
	...
	private TextView acounter=null;
	...
	private LinearLayout attachmentLayout, ll_forward_attachments_layout = null;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		acounter= (TextView) layout.findViewById(R.id.attach_count);
		...
		if (item != null) {
			ccText.setVisibility(View.INVISIBLE);
			bccText.setVisibility(View.INVISIBLE);
		}

		layout.findViewById(R.id.tv_addattachment).setOnClickListener(bsecure);

		ed_toText = (AutoCompleteTextView) layout.findViewById(R.id.ed_toText);
		/*
					...
					if (object.optString("status").equalsIgnoreCase("0")) {
						if (object.has("message_content")) {
							...
							//for (int i = 0; i < jsonArray.length(); i++) {
								JSONObject innertMessage = jsonArray.getJSONObject(0);
								builder.append("<br/>");
								//builder.append("\n");
...
//								builder.append("\n");
							...
							builder.append("\n");
							//builder.append("<br/>");
							ed_comText.setText(Html.fromHtml("<html><body>"+builder.toString()+"</html></body>"));
						}
	...
	public void addAttachmentView(Item fullItem, long attachSize) {
		...
		acounter.setText("Attachments:"+attachmentCounter+"/10");
		...
		((ScrollView) categories_layout).smoothScrollTo(0, ((ScrollView) categories_layout).getBottom() + 20);
	}
